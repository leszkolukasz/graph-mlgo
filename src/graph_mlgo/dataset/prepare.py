...
from graph_mlgo.constants import MAX_EDGES, MAX_IR_LEN, MAX_NODES
...
def _validation_worker(
    ...
    logger.remove()
    logger.add(LOG_FILE, rotation="50 MB", enqueue=True, level="DEBUG")
...
def is_valid(
    ...
    p = ctx.Process(
        ...
        args=(ir_text, ir_bitcode, max_nodes, max_edges, max_ir_len, queue),
    )
    p.start()
    p.join()
            ...
            def fill_futures():
                while len(futures) < max_workers * 2:
                    try:
                        ...
                        fut = executor.submit(
                            process_sample_task, idx, sample, apply_filter
                        )
                        futures.add(fut)
                    except StopIteration:
                        break
...
def prepare_dataset(
    ...
    save_path = f"{output_path}/ComPile-{size_GB}GB" + (
        "-parsed" if parse_bitcode else ""
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--size_GB", type=float, default=10, help="Size of the train dataset in GB"
    )
    parser.add_argument("--output_path", type=str, default="data", help="Path to save")
    parser.add_argument(
        "--parse_bitcode",
        action="store_true",
        help="Parse bitcode to text IR",
        default=False,
    )
    parser.add_argument(
        "--filter", action="store_true", help="Run sanity checks", default=True
    )
    ...
    prepare_dataset(
        ...
        parse_bitcode=args.parse_bitcode,
